...
test.describe.skip('Drag and Drop', () => {
  ...
  test('should drag card within same column', async ({ page }) => {
    ...
    const cards = todoColumn
      .locator('[data-testid^="card-"]')
      .or(todoColumn.locator('[class*="card"]'));
    ...
    // Verify card moved
    const cardsAfterDrag = await todoColumn
      .locator('[data-testid^="card-"]')
      .or(todoColumn.locator('[class*="card"]'));
  ...
  test('should drag card to different column', async ({ page }) => {
    ...
    await page.mouse.down();
    await page.mouse.move(targetColumnBox.x + targetColumnBox.width / 2, targetColumnBox.y + 100, {
      steps: 10,
    });
    await page.mouse.up();
    ...
    const inProgressCards = inProgressColumn
      .locator('[data-testid^="card-"]')
      .or(inProgressColumn.locator('[class*="card"]'));

    const movedCardExists = await inProgressCards
      .locator(`text=${cardText?.trim().substring(0, 20)}`)
      .isVisible();
    expect(movedCardExists).toBeTruthy();
  });

  test('should show drag handle on hover', async ({ page }) => {
    const cards = page.locator('[data-testid^="card-"]').or(page.locator('[class*="card"]'));
    ...
    const dragHandle = firstCard
      .locator('[class*="grip"]')
      .or(firstCard.locator('svg').filter({ hasText: /grip/i }));
  ...
  test('should update card position after drag', async ({ page }) => {
    ...
    const cardsAfterReload = todoColumn
      .locator('[data-testid^="card-"]')
      .or(todoColumn.locator('[class*="card"]'));
